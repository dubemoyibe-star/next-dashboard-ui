...
const EventForm = ({type, data, setOpen, relatedData}: EventFormProps) => {
  ...
  const onSubmit = handleSubmit(data => {
    ...
    startTransition(() => {
      ...
      const parsed = eventSchema.parse(formattedData)
      formAction(parsed)
    })
  })
  ...
  useEffect(() => {
    if(state.success) {
      toast(`Event has been succesfully ${type === "create" ? "created" : "updated" }!`)
      router.refresh()
      setOpen(false)
    }
 ...
 const { classes } = relatedData
  ...
  return (
    <div className=' flex flex-col max-h-[90vh]'>
      ...
      <h1 className='text-xl font-semibold text-gray-900 dark:text-gray-100'>{type === "create" ? "Create a new event" : "Update the event"}</h1>
